...
SECTION_HEARTBEAT = (
    ...
    "An empty HEARTBEAT.md means heartbeat runs are skipped entirely (no API cost). "
    "Write items when there is something to monitor; clear them when done.\n\n"
    "Use HEARTBEAT.md for transient reminders and monitors (PRs, deploys, "
    "time-sensitive checks). Use the task tracker for persistent work items "
