...
The 0.5.0 theme: agents that **plan visibly and learn durably**. A pluggable agent harness adds two new tiers above the react loop — `balanced` (a live todo-driven plan checklist at react cost) and `ultra` (triage → plan → execute → reflect with per-node models) — while a three-phase learning loop lets agents create and update their own skills behind validation gates. The existing react agent is untouched and remains the default — a 45-scenario feature-parity matrix pins every existing behavior across harness types.

### Added

- **Ultra agent harness** (`harness.type: ultra`): a triage node routes each message to the plain react loop (simple turns), a planning path (multi-step work), or an ideation path (open-ended asks). Plans are first-class state — checkpointed, revisable (step insertion "2A", remaining-plan rewrites), resumable across restarts, and rendered as a live edited-in-place checklist in-channel. Execution embeds the existing react loop per step, so middleware, approval gates, steer/interrupt, and status updates apply to every planned step by construction. Triage and every module call fail open to the react path — ultra is never less reliable than the current loop.
- **Balanced agent harness** (`harness.type: balanced`): the middle tier — the react loop plus a todo-driven live plan checklist at **zero extra harness LLM calls**. The agent maintains its plan via a `write_todos` tool (statuses `pending`/`in_progress`/`completed`/`failed`, plus an optional one-line `note`), and a bridge diffs every write into the same live edited-in-place checklist ultra renders — failed steps get a first-class ✗ and notes surface as one-line progress updates, all riding inside turns the model was already taking. Ships with `explore_lenses`, a deterministic zero-LLM ideonomy tool for open-ended asks (`harness.ideation`), and optional checkpoint reflection (`harness.reflection.mode: checkpoint`): a structured verdict every N completed steps — immediately on a failed one — injected as a course-correction nudge; the organic default costs nothing. The checklist survives auto-compact: current todos are re-injected into the fresh conversation. Config: `harness.plan.visibility`, `harness.ideation`, `harness.reflection`.
...
- **Context brief** (`harness.brief`, on by default): ultra planning and ideation runs now open with a `brief` node that distills the full session history — token-budgeted against the brief model's context window rather than a fixed cutoff — into a structured brief consumed by the reasoning modules, step execution, synthesis, and tool equipping, so plans reflect the whole conversation instead of a snippet. Simple react turns skip it entirely; any brief failure falls open to the lightweight digest. Config: `enabled`, `model`, `temperature`, `max_tokens`, `max_input_tokens`.
- **Per-node models** (ADR-103): each harness node (triage, planning, creative, reflection, selector, synthesize) can point at a provider-catalog entry (`triage: {model: fast}`) — credentials stay in the catalog, provider quirks stay in `create_chat_model()`, and a bare `harness: {type: ultra}` runs with every node inheriting the workspace model. `/harness` prints the resolved node→model table.
...
- Harness tier naming locked as **react / balanced / ultra**. (The top tier was briefly named `planner` during development; all config values, identifiers, and telemetry use `ultra` — no compatibility alias since 0.5.0 never shipped.)
- **The ultra harness narrates its phases in the status line** (round-1 testing feedback): "Thinking..." during triage, a first-person routing announcement after it ("I need to form a plan for this..." / "I need to think about this creatively..."), which planning/creative module was selected, per-step reflection outcomes ("going back to the drawing board..." on a plan rewrite), and "Putting it all together..." while synthesizing. Harness runs suppress the generic "Starting work..." label, and harness phase lines bypass the status throttle so transitions are never silently dropped. React-only workspaces are unchanged.
- **The balanced harness shows "Considering..." at run start** instead of the generic "Starting work...", reflecting that its first turn decides whether to answer directly or start a plan. React workspaces keep "Starting work..."; ultra keeps suppressing the generic label in favor of its phase narration.
- **The balanced harness announces a first-person intent before module specifics** (same two-tier UX as ultra's post-triage route line): "I need to form a plan for this..." precedes the plan checklist, and "I need to think about this creatively..." precedes the lens exploration. The intent is inferred at tool-call time in the plan-event bridge (balanced has no triage node) and fires at most once per run per session, reusing ultra's `_ROUTE_LINES` vocabulary.
...
- **Ideonomy and Self-Discover are now compiled LangGraph subgraphs**: ideonomy's lens explorations run concurrently instead of one at a time, and every Self-Discover discovery stage (SELECT/ADAPT/IMPLEMENT) uses structured outputs — the silent freeform-JSON fallback is gone. The reasoning-structure cache format is unchanged.
- The fallback conversation digest passed to reasoning modules is now role-labeled and dialogue-only (tool traces excluded), so modules see who said what instead of an unlabeled text slice.
...
- **Status-reminder middleware now recognizes real user deliveries and guards against tool-spin runaways.** The silent-run counter resets on any genuine delivery tool (`send_message`, `send_file`, `request_followup`) rather than `send_message` alone — `text_to_speech`, which only *stages* audio, no longer wrongly silences the reminder. A new config-gated anti-spin guard (`status_reminder.repeat_tool_limit`, default 6; `repeat_guard_max`, default 2) nudges the agent when it calls the same single tool many times in a row without progressing or updating the user, ending the class of runaway loop where an agent repeated one staging tool dozens of times. It is a nudge — the `recursion_limit` remains the hard backstop.
...
- **The balanced plan checklist keeps updating across turns** — the final step tick is no longer lost when a plan spans multiple messages. The plan renderer's state used to be reset on every run boundary; a continuing turn's bare step event now edits the same checklist message (`plan.created` remains the sole reset trigger, so ultra still starts a fresh checklist per run).
